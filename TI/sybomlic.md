```
导入库，打印，进程，文件，
行为：收集电脑信息，fork程序，设置自启，隐藏（删除自身文件），函数（fork,getpid,delete file）
virustotal搜索RotaJakiro木马
```
...
CLE模块载入二进制文件，然后反汇编生成汇编码并根据jmp等指令进行block的划分。（反汇编为哪一种架构呢看，如何识别的？），通过元信息识别架构，二进制转为汇编。

生成VEX中间语言，该语言类似cpu语言，可进行模拟执行。（怎么模拟执行，汇编不行嘛，如何由汇编映射到中间语言的）。汇编不通用，不同架构要分别编写，VEX比汇编粒度小，一条汇编转为多条VEX IR。
...
```
栈地址由高向低增长，地址单位为一个字节，即读取单个地址的数值显示为8位二进制，即两位16进制。esp指向的不是之前push的eax，因为进入fopen还要push ebp，保存栈底指针。
```
