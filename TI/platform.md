...
>恶意软件和报告哪来，网上挖掘。
>
>从物联网cve开始向右，每一块是什么，已经有多少条了。点开查看，有意义的条可以说明一下，如springboot漏洞，fastjson漏洞，apt报告的恶意软件。

### mr爬虫

>输入map的是什么，是单条url嘛，那么map用来干什么，爬取放在那一步？mr的意义是什么，什么情况下mr有意义？reduce可能会有倾斜的情况，所以map爬取比较好。map判断直接爬取，输入就是所需url和其他参数信息。在mr能缩减执行时间，能方便快速开发并行程序时，时有意义的。在爬虫这个场景下，如果划分的maptask执行时间比较长，那么就是有意义的，地区加快了爬取速度，当某一个map函数失败时，他会重试嘛？重试的单位是什么，一个maptask？对应一个tasktracker？是以maptask为单位的，可以控制重试次数，和容忍的失败task百分比。
...
|               | 主键检索 | 非主键检索（全文检索） | 聚合统计 | 多表联查 |
| ------------- | -------- | ---------------------- | -------- | -------- |
| mysql         | 适合     | 不适合                 | 不适合   | 适合     |
| hbase         | 适合     | 不适合                 | 适合     | 适合     |
| elasticsearch | 适合     | 适合                   | 适合     | 不适合   |
| hive          | 不适合   | 不适合                 | 适合     | 适合     |

```
elasticsearch不支持复杂的多表join，有时候单表的复杂统计也不好写，只能用painless通过自定义key值进行聚合统计。且es不支持查询结果的存储和额外分析
```
