...
>与决策树，随机森林，gcforest相比，S-gcForest的精度。

```
其中rank loss，accuracy可用，macro f1过低。
```
...
```
#yeast数据集，10折交叉，窗口30 10,num_forst=8
...
time=1085.12s  18.09min   speedup_single=4.65x  speedup=4.68x
mgs: 1.94   cf: 16.15          mgs_speedup=3.1x     cf_speedup=4.83x
...
#准确率
#random_state=3
('hamming loss', 0.33355014191106913)
('one-error', 0.2367549668874172)
('coverage', 0.4634578997161779)
('ranking loss', 0.17900559016039191)
('average precision', 0.7445333870105123)
('macro-f1', 0.445405213213823)
('macro-auc', 0.670237038202825)

#random_state=0
('hamming loss', 0.33780747398297073)
('one-error', 0.2367549668874172)
('coverage', 0.46168401135288556)
('ranking loss', 0.17930198841681802)
('average precision', 0.7443815172206145)
('macro-f1', 0.43572113777786436)
('macro-auc', 0.6648476024376745)
```

#### scene数据集

```
#scene数据集，10折交叉，窗口30 10,num_forst=8

#5个
time=700.63s  11.68min  
mgs:2.54 cf: 9.14     

time=694.45s  11.57min
mgs:2.54 cf: 9.03    
#4个
time=889.23s   14.82min   speedup=3.50x

#3个
time=1077.32s  17.95min     speedup=2.89x

#2个
#刚好2.0x time=1,558.5  
time=1,564.5s    26.075      speedup=1.99x
time= 1546.85s   25.78min    speedup=2.015x

#1个   
time=3117.89   51.96min    

#single
time=3121.47s  52.02min
mgs:8.73  cf:43.29
```
...
```
#参数设置：enron数据集,10折交叉，窗口300  100 ,num_forest=4

#5个
time=1288.61s       21.48min    speedup_single=3.71x  speedup=3.74x
mgs:2.11  cf:19.37   mgs_speedup=      cf_speedup= 

time=1097.55s       18.29min 

#4个
time=1492.33s       24.87min      speedup=3.23x

#3个
time=1706.48s       28.44min      speedup=2.82x

#2个
#刚好2x，time=2410s  
time=2417.20s  40.28min  speedup=1.99x
time=2396.20s       39.93min      speedup=2.01x

#1个
time=4820.78s      80.34min       

#single
time=4777.13s      79.62min  
mgs：6.38  cf:73.24
```
...
```
#参数设置：genbase数据集,10折交叉，窗口300  100 ,num_forst=8

#5个
time=597.88s  9.96min
mgs:1.71 cf:8.25

#4个
time=764.79s  12.74min

#3个
time=915.69   15.26min

#2个
time=1313.39s  21.88min

#1个
time=2616.85s   43.61min 

#single 
time=2563.82     42.73min
mgs:5.95  cf:36.78
```
...
```
#参数设置：cal500数据集,10折交叉，窗口20  6 ,num_forst=2

#5个
time=958.94s    15.98min       speedup=3.25x

#4个                       
time=912.87s     15.21min      speedup=
...
```
#5个
time=1776.15s 29.60min  
time=1266.77s 21.11min  
```
...
```
#5个 使用1:1权重    
#spark log:app-20220529205408-0066
time=1162.43  19.37min   

#5个 不使用重分区
time=1122.40  18.70

#5个 使用2:1权重
time=1085.12s  18.08min
```
...
```
time=1063.12s  17.71min
time=1107.47   18.45min
```
