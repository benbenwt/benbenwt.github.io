...
202206
13-15 实现df的mr版本
16 确认软件园的程序版本，以及需要添加修改哪些。
16 把docker跑起来，确认功能正常
16 编写基础镜像的dockerfile文件
16 指定固定的版本，不然es和python库都会有
16 复制需要的镜像和代码、资源
16 
x  异常与错误，错误的输出。
x  单机randomstate重做确认
x  学校的这个项目能否包装，主要是数据量的问题，然后是来源问题，因为其本质是爬虫项目，所以业务不会太复杂。
x  主要是文章，ti，大数据的代码
x  文章主要ner的和摸索的那些，ner的有一些大文件无法上传，以其他形式记录好，比如资源链接。
x  总结之前的article代码
x  整理项目代码、论文代码到github和移动磁盘,angr  实验室电脑、移动磁盘、远程仓库
...
25-29 完成消融的，完成与DT、randomforest、extraTree的精度对比
